...
describe('Chek grid filters', function () {
    before(() => {
    ...
    it('chek status filter', function () {

   
      cy.getGridByTestId('desktop_grid')
      .invoke('getColumns')
      .then((num) => {
        
       return num.filter(word => word.getDataIndex() =='DocumentDirectionFilters')[0].getFilterType().field;})
       .then((num) => {
         cy.wrap(num)
       .its('el')
       .its('dom').find('.x-expandtrigger').click()
      cy.get('.x-menucheckitem').eq(3).click()
      cy.wait(2000)
      cy.getButtonByTestId('filterbutton').clickButton()
      ...
      cy.wrap(num)
      .its('el')
      .its('dom').find('input').should('have.value', 'Отклонены')
      
      
      })
      
      
     // cy.get('[data-cy=top_menu_container]').find('h2').contains('Входящие')x-menucheckitem
